...
if ( !function_exists( 'futurio_widget_date_comments' ) ) :
	...
	function futurio_widget_date_comments() {
		...
		<?php } ?>
		<?php if ( !comments_open() && get_theme_mod( 'blog_archive_comments', 'on' ) == 'on' || is_singular() ) { ?>
...
if ( class_exists( 'WooCommerce' ) ) {
	...
	function futurio_main_content_width_woo_columns() {
		...
		$columns = '12';
		if ( ( is_shop() || is_product_tag() || is_product_category() ) && is_active_sidebar( 'futurio-woo-archive-sidebar' ) ) {
			$columns = $columns - absint( get_theme_mod( 'sidebar_size', '3' ) );
		} elseif ( is_active_sidebar( 'futurio-woo-right-sidebar' ) ) {
			$columns = $columns - absint( get_theme_mod( 'sidebar_size', '3' ) );
		}
...
if ( !function_exists( 'futurio_get_meta' ) ) :
	...
	function futurio_get_meta( $name = '', $output = '' ) {
		if ( is_singular() ) {
			...
			if ( isset( $meta ) && $meta != '' && is_singular() ) {
				if ( $output == 'echo' ) {
					echo esc_html( $meta );
				} else {
					return $meta;
